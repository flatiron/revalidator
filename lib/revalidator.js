  ...
  function validateProperty(object, value, property, schema, options, errors, path) {
    ...
    if (value === undefined) {
      if (schema.required && schema.type !== 'any') {
        return error('required', property, undefined, schema, errors, path);
      } else {
        return;
      }
    }
      ...
      switch (type || (isArray(value) ? 'array' : typeof value)) {
        ...
        case 'array':
          constrain('items', value, function (a, e) {
            for (var i = 0, l = a.length; i < l; i++) {
              validateProperty(object, a[i], property, e, options, errors, path);
            }
            return true;
          });
        ...
        case 'object':
          ...
          if (schema.properties || schema.patternProperties || schema.additionalProperties) {
            validateObject(value, schema, options, errors, property + ".");
          }
          break;
      }
    });
